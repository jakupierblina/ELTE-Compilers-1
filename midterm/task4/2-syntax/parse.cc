// Generated by Bisonc++ V5.02.00 on Mon, 19 Oct 2020 18:43:48 +0200
...
PI__ const s_productionInfo[] = 
{
     ...
     {296, 5}, // 6: declaration (T_LESS) ->  T_LESS tuple T_GR T_ID T_SEMICOLON
     {298, 1}, // 7: type (T_INTEGER) ->  T_INTEGER
     {298, 1}, // 8: type (T_BOOLEAN) ->  T_BOOLEAN
     {297, 1}, // 9: tuple ->  type
     {297, 3}, // 10: tuple (T_COMMA) ->  type T_COMMA tuple
     {295, 1}, // 11: statements ->  statement
     {295, 2}, // 12: statements ->  statement statements
     {299, 2}, // 13: statement (T_SKIP) ->  T_SKIP T_SEMICOLON
     {299, 1}, // 14: statement ->  assignment
     {299, 1}, // 15: statement ->  read
     {299, 1}, // 16: statement ->  write
     {299, 1}, // 17: statement ->  branch
     {299, 1}, // 18: statement ->  loop
     {305, 3}, // 19: access_tuple (T_ID) ->  T_ID T_POINT T_NUM
     {300, 4}, // 20: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {300, 4}, // 21: assignment (T_ASSIGN) ->  access_tuple T_ASSIGN expression T_SEMICOLON
     {301, 5}, // 22: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {301, 5}, // 23: read (T_READ) ->  T_READ T_OPEN access_tuple T_CLOSE T_SEMICOLON
     {302, 5}, // 24: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {303, 5}, // 25: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {303, 7}, // 26: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {304, 5}, // 27: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {306, 1}, // 28: expression (T_NUM) ->  T_NUM
     {306, 1}, // 29: expression (T_TRUE) ->  T_TRUE
     {306, 1}, // 30: expression (T_FALSE) ->  T_FALSE
     {306, 1}, // 31: expression (T_ID) ->  T_ID
     {306, 3}, // 32: expression (T_ADD) ->  expression T_ADD expression
     {306, 3}, // 33: expression (T_SUB) ->  expression T_SUB expression
     {306, 3}, // 34: expression (T_MUL) ->  expression T_MUL expression
     {306, 3}, // 35: expression (T_DIV) ->  expression T_DIV expression
     {306, 3}, // 36: expression (T_MOD) ->  expression T_MOD expression
     {306, 3}, // 37: expression (T_LESS) ->  expression T_LESS expression
     {306, 3}, // 38: expression (T_GR) ->  expression T_GR expression
     {306, 3}, // 39: expression (T_EQ) ->  expression T_EQ expression
     {306, 3}, // 40: expression (T_AND) ->  expression T_AND expression
     {306, 3}, // 41: expression (T_OR) ->  expression T_OR expression
     {306, 2}, // 42: expression (T_NOT) ->  T_NOT expression
     {306, 3}, // 43: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {306, 1}, // 44: expression ->  access_tuple
     {307, 1}, // 45: program_$ ->  program
};
